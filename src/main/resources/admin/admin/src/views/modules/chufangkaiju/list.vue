			...
			<!-- <div> -->
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button :style='{"border":"1px solid #85bfeb","cursor":"pointer","padding":"0 10px 0 26px","margin":"0 5px 6px","outline":"none","color":"#1e98f5","borderRadius":"20px","background":"url(http://codegen.caihongy.cn/20230329/38553f099fa74a2d86e2206c373a9539.png) no-repeat 5px center / 20px","width":"auto","fontSize":"15px","height":"32px"}' v-if=" isAuth('chufangkaiju','查看')" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
							<el-button :style='{"border":"1px solid #a4dbdd","cursor":"pointer","padding":"0 10px 0 26px","margin":"0 5px 6px","outline":"none","color":"#15a4aa","borderRadius":"20px","background":"url(http://codegen.caihongy.cn/20230329/99f867042acb4c4aae637c8bfcf6714a.png) no-repeat 3px center","width":"auto","fontSize":"15px","height":"32px"}' v-if=" isAuth('chufangkaiju','修改')" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>





							<el-button :style='{"border":"1px solid #e68080","cursor":"pointer","padding":"0 10px 0 26px","margin":"0 5px 6px","outline":"none","color":"#c00","borderRadius":"20px","background":"url(http://codegen.caihongy.cn/20230329/c5ea125f385740a3a3b9aed07f5dfe2c.png) no-repeat 5px center / 20px","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('chufangkaiju','删除') " type="danger" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
						</template>
					</el-table-column>
				</el-table>
				<el-pagination
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        ...
        this.$http({
            url: `chufangkaiju/value/kaijushijian/shuliang/月`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].kaijushijian);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].kaijushijian
                    })
                }
                var option = {};
                option = {
